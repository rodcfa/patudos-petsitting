/*
  ...
  1. Novas Tabelas
    ...
    - `bookings` - Reservas/agendamentos com status de aprovação
    - `booking_pets` - Relacionamento entre reservas e pets

  2. Segurança
    - Enable RLS em todas as tabelas
    - Políticas baseadas em autenticação e roles
    - Separação entre dados de clientes e administradores

  3. Funcionalidades
    - Sistema de aprovação de reservas
    - Múltiplos pets por reserva
    - Dados completos dos pets (saúde, comportamento, etc.)
...
BEGIN
  -- Buscar ID do admin
  SELECT id INTO admin_id FROM users WHERE email = 'admin@patudos.com';
  ...
  -- Se o cliente já existe, buscar o ID
  IF client_id IS NULL THEN
    SELECT id INTO client_id FROM users WHERE email = 'cliente@exemplo.com';
  END IF;
  ...
  VALUES 
    (client_id, 'Rex', 'Golden Retriever', 3, 'male', 30.5, true, 'yes', 'yes', 'Ração premium 2x ao dia, 200g cada refeição'),
    (client_id, 'Luna', 'Border Collie', 2, 'female', 22.0, true, 'yes', 'depends', 'Ração para cães ativos 2x ao dia, 150g cada refeição')
  ON CONFLICT DO NOTHING
  RETURNING id INTO pet1_id;
  
  -- Buscar IDs dos pets se já existem
  ...
  -- Se a reserva já existe, buscar o ID
